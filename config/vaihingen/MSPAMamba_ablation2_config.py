...
from ours_env.datasets.vaihingen_dataset import *
...
weights_name = "mspamamba-ablation2-U"
weights_path = "model_weights/vaihingen/{}".format(weights_name)
test_weights_name = "mspamamba-ablation2-U"
...
# ====================== 2. 模型配置 ======================
# 消融实验2：移除MSMamba和CAFMamba，保留MPAM
from ours_env.models.MSPAMamba_ablation2 import MSPAMamba, load_pretrained_ckpt
...
if 'train_supervision.py' in sys.argv[0] or 'train' in sys.argv:
    try:
        ...
        print("Successfully loaded pretrained VMamba weights for ablation2")
    except Exception as e:
        print(f"Warning: Could not load pretrained VMamba weights: {e}")
        print("Training ablation2 from scratch.")
else:
...
def val_aug(img, mask):
    img, mask = np.array(img), np.array(mask)
    aug = albu.Compose([albu.Normalize()])(image=img, mask=mask)
    img, mask = aug['image'], aug['mask']
    return img, mask
