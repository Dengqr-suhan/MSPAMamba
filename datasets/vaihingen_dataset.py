...
# 根据您的需求和实际数据分布设置
# 训练区域：您指定的train_ids对应的区域
TRAIN_AREA_IDS = ['1', '3', '23', '26', '7', '11', '13', '28', '17', '32', '34', '37']
# 测试区域：从测试集中选择一些区域
TEST_AREA_IDS = ['2', '4', '6', '8']  # 这些区域在测试集中存在
...
class VaihingenDataset(Dataset):
    def __init__(self, data_root='data/vaihingen/test', mode='val', img_dir='images_1024', mask_dir='masks_1024',
                 ...
                 img_size=ORIGIN_IMG_SIZE, specified_area_ids=None):
        ...
        self.specified_area_ids = specified_area_ids
    ...
    def get_img_ids(self, data_root, img_dir, mask_dir):
        # 如果指定了特定的区域ID列表，使用该列表
        if self.specified_area_ids is not None:
            target_area_ids = self.specified_area_ids
        # 如果没有指定，根据mode自动选择
        elif self.mode == 'train':
            target_area_ids = TRAIN_AREA_IDS
        elif self.mode == 'test' or self.mode == 'val':
            target_area_ids = TEST_AREA_IDS
        else:
            # 如果mode不是train/test/val，则使用原来的逻辑（扫描目录）
        ...
        # 根据指定的区域ID，扫描目录找到对应的文件
        img_ids = []
        img_dir_path = osp.join(data_root, img_dir)
        mask_dir_path = osp.join(data_root, mask_dir)
        
        if os.path.exists(img_dir_path) and os.path.exists(mask_dir_path):
            img_files = os.listdir(img_dir_path)
            mask_files = os.listdir(mask_dir_path)
            
            for area_id in target_area_ids:
                # 查找包含指定区域ID的文件
                area_pattern = f'top_mosaic_09cm_area{area_id}_'
                area_img_files = [f for f in img_files if f.startswith(area_pattern) and f.endswith(self.img_suffix)]
                area_mask_files = [f for f in mask_files if f.startswith(area_pattern) and f.endswith(self.mask_suffix)]
                
                # 确保图像和掩码文件匹配
                for img_file in area_img_files:
                    img_id = img_file.replace(self.img_suffix, '')
                    mask_file = img_id + self.mask_suffix
                    if mask_file in area_mask_files:
